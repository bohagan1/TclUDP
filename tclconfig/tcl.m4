...
AC_DEFUN([TEA_PATH_TCLCONFIG], [
	...
	AC_CACHE_VAL(ac_cv_c_tclconfig,[
		...
		if test x"${PKG_CONFIG}" != x":" ; then
		    for i in tcl tcl9.1 tcl9.0 tcl8.6 tcl8.5; do
		...
		if test x"${PKG_CONFIG}" != x":" ; then
		    for i in tk tk9.1 tk9.0 tk8.6 tk8.5; do
			if ${PKG_CONFIG} --exists $i 2>/dev/null; then
			    pkgconf_libdir=`${PKG_CONFIG} --variable=libdir $i`
		...
		for i in `ls -d ${libdir} 2>/dev/null` \
			...
			`ls -d /usr/lib/*-linux-gnu/tcl9.1 2>/dev/null` \
...
AC_DEFUN([TEA_PATH_TKCONFIG], [
	...
	AC_CACHE_VAL(ac_cv_c_tkconfig,[
		...
		for i in `ls -d ${libdir} 2>/dev/null` \
			...
			`ls -d /usr/lib64 2>/dev/null` \
			`ls -d /usr/lib/*-linux-gnu 2>/dev/null` \
			`ls -d /usr/lib/*-linux-gnu/tk9.1 2>/dev/null` \
